              ...
              {items.length === 0 ? (
                ...
                <AnimatePresence initial={false}>
                  {items.map((product) => {
                    ...
                    // Same redirect-out rule as the Hub cards: prefer the
                    // product's own landing page, fall back to the internal
                    // product page only if no external URL is set.
                    const externalUrl =
                    ...
                    return (
                      ...
                      >
                        {/* Thumbnail */}
                        {linkAttrs ? (
                          <a {...linkAttrs} onClick={closeWishlist} className="relative w-14 h-14 rounded-lg overflow-hidden shrink-0 cursor-pointer">
                            {thumb}
                          </a>
                        ) : (
                          <Link to={`/product/${product.slug}`} onClick={closeWishlist} className="relative w-14 h-14 rounded-lg overflow-hidden shrink-0 cursor-pointer">
                            {thumb}
                          </Link>
                        )}
                        ...
                        <div className="flex-1 min-w-0">
                          {linkAttrs ? (
                            <a {...linkAttrs} onClick={closeWishlist} className="block text-sm font-medium text-foreground hover:text-primary transition-colors truncate cursor-pointer">
                              {product.name}
                            </a>
                          ) : (
                            <Link to={`/product/${product.slug}`} onClick={closeWishlist} className="block text-sm font-medium text-foreground hover:text-primary transition-colors truncate cursor-pointer">
                              {product.name}
                            </Link>
                          )}
