...
type Row = ProductStatusDoc & { usageCount: number };

export default function ProductStatusesPanel() {
  const rows = useQuery(api.productStatuses.listWithUsage);
  const create = useMutation(api.productStatuses.create);
  const update = useMutation(api.productStatuses.update);
  const remove = useMutation(api.productStatuses.remove);
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-card p-5 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h3 className="font-semibold text-sm text-foreground">Product Statuses</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Controls Hub visibility, purchasability, and CTA behavior per status — not hard-coded.
          </p>
        </div>
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search statuses…" className="pl-9 h-9" />
      </div>

      <div className="space-y-2">
        {rows === undefined && <p className="text-xs text-muted-foreground py-4 text-center">Loading…</p>}
        {rows && filtered.length === 0 && (
          <p className="text-xs text-muted-foreground py-4 text-center">No product statuses match.</p>
        )}
        {filtered.map((row) => (
          <div key={row._id} className="flex items-center gap-3 p-3 rounded-xl border border-white/8 bg-background/50">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">
                {row.slug} · {row.usageCount} product{row.usageCount === 1 ? "" : "s"} ·{" "}
                {row.isPublic ? "Public" : "Hidden"} · {row.isPurchasable ? "Purchasable" : "Not purchasable"} · CTA {row.isCtaEnabled ? "on" : "off"}
              </p>
            </div>
            ...
            <button
              ...
              title={row.isSystem ? "System statuses can't be deleted — deactivate instead" : "Delete"}
        ...
        <ProductStatusFormDialog
          ...
          onSave={async (values) => {
            try {
              if (editing === "new") {
                await create(values);
                toast.success("Product status created");
              } else {
...
function ProductStatusFormDialog({ initial, onClose, onSave }: {
  initial?: Row;
  onClose: () => void;
  onSave: (values: {
  ...
  const [badgeVariant, setBadgeVariant] = useState(initial?.badgeVariant ?? "");
  const [displayOrder, setDisplayOrder] = useState(String(initial?.displayOrder ?? 0));
  const [isPublic, setIsPublic] = useState(initial?.isPublic ?? false);
  const [isPurchasable, setIsPurchasable] = useState(initial?.isPurchasable ?? false);
  const [isCtaEnabled, setIsCtaEnabled] = useState(initial?.isCtaEnabled ?? false);
  ...
  const handleSave = async () => {
    ...
    await onSave({
      ...
      badgeLabel: badgeLabel.trim() || undefined, badgeVariant: badgeVariant.trim() || undefined,
      displayOrder: Number(displayOrder) || 0, isPublic, isPurchasable, isCtaEnabled,
    });
    setSaving(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 py-8 overflow-y-auto">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-card p-5 space-y-4 my-auto">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-sm text-foreground">{initial ? "Edit product status" : "New product status"}</h3>
        ...
        <div className="space-y-1.5">
          ...
          <Input value={slug} onChange={(e) => { setSlug(e.target.value); setSlugTouched(true); }} placeholder="waitlist" />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Input value={badgeLabel} onChange={(e) => setBadgeLabel(e.target.value)} placeholder="e.g. Waitlist Open" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Badge variant</Label>
            <Input value={badgeVariant} onChange={(e) => setBadgeVariant(e.target.value)} placeholder="info / muted / primary" />
          </div>
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-2 pt-1">
          <div className="flex items-center justify-between rounded-lg border border-white/10 px-3 py-2">
            <div>
              <p className="text-sm text-foreground">Public visibility</p>
              <p className="text-[11px] text-muted-foreground">Products with this status appear on the public Hub</p>
            </div>
            <Switch checked={isPublic} onCheckedChange={setIsPublic} />
          </div>
          <div className="flex items-center justify-between rounded-lg border border-white/10 px-3 py-2">
            <div>
              <p className="text-sm text-foreground">Purchasable</p>
              <p className="text-[11px] text-muted-foreground">Informational — actual checkout happens on the landing page</p>
            </div>
            <Switch checked={isPurchasable} onCheckedChange={setIsPurchasable} />
          </div>
          <div className="flex items-center justify-between rounded-lg border border-white/10 px-3 py-2">
            <div>
              <p className="text-sm text-foreground">CTA enabled</p>
              <p className="text-[11px] text-muted-foreground">Whether the card's CTA button is clickable</p>
            </div>
